...
> 	对于32位的系统，能使用的最大地址空间是4GB，其中的地址空间0~3GB分配个用户进程使用，地址空间3~4GB空间分配给内核使用。但是用户进程并不是在启动时就获得了对所有0~3GB的地址空间访问权限，而是需要实现向内核申请对某块地址空间的读写权利。而且申请只是地址空间而已，此时并没有分配真实的物理内存。只有当进程访问某个地址时，如果改地址对应的物理页面不存在，则由内核产生缺页中断，在中断中才会分物理内存并建立起来页表。如果用户进程不需要某块空间了，可以通过内核释放它们，对应的物理内存也将同时释放掉。
>
> 	但是缺页中断运行慢，如果频繁的由内核来分配和释放内存将会降低这个系统的性能，因此，一般操作系统都会在用户进程中提供地址空间的分配回收机制。用户进程中的内存管理器会预先向内核申请一块大的地址空间，称为堆。当用户进程需要分配内存时，有内存管理器从堆中寻找一块空闲的内存分配给用户进程使用。当用户进程释放某块内存时，内存管理器并不会立刻将它们交给内核释放，而是重新放入到空闲列表中，留待下次分配使用，也就是回收到堆中了。

##### 3.3.1 系统调用 brk和 mmap

##### 3.3.2 内存分配器-dlmalloc简介

> dlmalloc中用来管理进程的"堆"空间的函数都带有“dl”前缀，而私有堆的管理函数带有函数前缀“msspace_”

#### 3.4 管道

> 管道（pipo）是从unix系统出现的一种进程间通信的手段，分为匿名管道和命名管道两种。

##### 3.4.1 匿名管道pipe和命令管道fifo

> 	匿名管道主要用于父子进程间的通信，这和匿名管道的实现原理有关系。创建匿名管道会在内核建立一个内存文件和两个文件描述符。管道使用者通过这两个文件描述符来读写文件中的数据，从而达到信息交换吧的目的。但是通常情况下问价描述符不能再进程间传递(Binder能够实现)，只有父子进程或兄弟进程间可以通过继承的方式来共享文件描述符，因此，匿名管道只能用于父子进程间通信。
>
> 	管道是半双工的，数据只能从一端到另一端。管道两端的进程将管道看成同一个文件，一个进程负责向管道中写，另一个负责从管道往外读取。这种数据传输方式遵循“先进先出”的队列原则。

![1535901966170](/ing/1535901966170.png)
...
![1535902335740](/ing/1535902335740.png)
...
3.7.2 native层的代码 android_util_Log.cpp 

```
\\192.168.1.8\share\lollipop-5.1.1_r6-release_3rd\frameworks\base\core\jni\android_util_Log.cpp
```
